...
class NsmcProcessor(object):
    ...
    def get_examples(self, mode):
        ...
        if mode == 'train':
            ...
            file_to_read = "question.txt"

        with open("question.txt", 'w', encoding= "utf-8") as f:
            print("question.txt 열렸다.")

        logger.info("LOOKING AT {}".format(os.path.join(self.args.data_dir, self.args.task, file_to_read)))
        if mode == 'pred':
            result = self._create_examples(self._read_file(file_to_read), mode)
        else:
            result = self._create_examples(self._read_file(os.path.join(self.args.data_dir,
                                                                  self.args.task,
                                                                  file_to_read)), mode)
        return result
